...
import { ArrowRight, Layers2, TrendingDown, TrendingUp } from 'lucide-react'
import React from 'react'
import 'dayjs/locale/es-mx'
import { useNavigate } from 'react-router-dom'
dayjs.locale("es-mx")

export const Transactions = ( {label, path, type, transactions} ) => {
                ...
                { transactions && transactions.slice(0, 3).map( t => (
                    <div key={t.id} className='hover:bg-gray-50 rounded-lg p-4 flex items-center gap-4'>
                        ...
                        <div className='flex flex-1 items-center justify-between'>
                            ...
                            <div className={`flex items-center gap-2 py-1 px-3 font-semibold text-sm border-1 rounded-sm ${type == "incomes" ? "bg-emerald-50 text-emerald-800 border-emerald-100" : "bg-rose-50 text-rose-800 border-rose-100"}`}>
                                <p>S/ {t.amount}</p>
                                {type == "incomes" ? <TrendingUp size={15} /> : <TrendingDown size={15} />}
                            </div>
